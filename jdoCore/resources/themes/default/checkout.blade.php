                ...
                <div class="col-lg-7">
                    ...
                    @php
                        ...
                        if ($pmWalletEnabled && auth()->check()) {
                            ...
                            $pmWallet = true; // tampilkan opsi (disabled jika saldo kurang)
                        }
                            ...
                            @if($pmWallet)
                                ...
                                <div class="form-check mb-2 {{ $walletEnough ? '' : 'opacity-75' }}">
                                    ...
                                    <label class="form-check-label d-flex flex-wrap align-items-center gap-2" for="pm-wallet">
                                        ...
                                        <span class="badge bg-primary-subtle text-primary fw-semibold">Saldo Anda: Rp{{ number_format($walletBalance, 0, ',', '.') }}</span>
                                    </label>
                                    ...
                                    <div class="alert alert-warning small mt-2 mb-0 py-2 border-0 d-none" id="pm-wallet-insufficient" style="border-radius:10px">
                                        Saldo Anda belum cukup untuk pembayaran ini. <a href="{{ route('account.wallet') }}">Top up sekarang</a>.
                                    </div>
                                </div>
                            @endif
                        </div>
    ...
    <script>
        function checkoutForm() {
            ...
            return {
                ...
                paymentMethod: '{{ $defaultChecked ?? '' }}',
                walletBalance: {{ $walletBalance ?? 0 }},
                walletEnabled: {{ ($pmWallet ?? false) ? 'true' : 'false' }},
                ...
                get canSubmit() {
                    if (this.paymentMethod === 'wallet') {
                        if (!this.walletEnabled) return false;
                        if (this.walletBalance < this.finalTotal) return false;
                    }
        ...
        // Sync paymentMethod into Alpine state + show wallet warning when balance insufficient
        document.querySelectorAll('input[name="payment_method"]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                var alpineRoot = document.querySelector('[x-data*="checkoutForm"]');
                if (alpineRoot && alpineRoot._x_dataStack) {
                    alpineRoot._x_dataStack[0].paymentMethod = this.value;
                }

                var warn = document.getElementById('pm-wallet-insufficient');
                if (!warn) return;

                if (this.value === 'wallet' && alpineRoot && alpineRoot._x_dataStack) {
                    var state = alpineRoot._x_dataStack[0];
                    if (state.walletBalance < state.finalTotal) {
