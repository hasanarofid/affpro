{{--
    Storefront Promo Popup
    ----------------------------------------------------------------
    Lazily fetches the active popup via /api/promo-popup/active and
    renders a centered modal-like overlay with a single image.
    Honors `display_delay` (s) and `show_once_per_session` from server.
...
<style>
    ...
    .pp-close:hover { background: rgba(0,0,0,.8); transform: rotate(90deg); }
    .pp-cta {
        padding: 14px 16px; text-align: center; border-top: 1px solid #f1f5f9;
    }
    .pp-cta a {
    ...
    .pp-cta a:hover { background: #1d4ed8; }
</style>
...
(function () {
    ...
    function build(popup) {
        ...
        const ctaHtml = popup.link_url
            ? `<div class="pp-cta"><a ${linkOpenAttrs}>${popup.button_label || 'Lihat Promo'}</a></div>`
            : '';
        ...
        root.innerHTML = `
            <div class="pp-card" role="dialog" aria-modal="true" aria-label="${popup.title || 'Promo'}">
                <button type="button" class="pp-close" aria-label="Tutup popup">&times;</button>
